...
use crate::index::lucene::documents::aggregate::{Include, Match};
...
use crate::index::lucene::documents::rules::{IndexingRule, IndexingRules, PropertyDefinition};
    ...
    fn index_property(
        ...
        if property.property_type == PropertyType::Binary {
            if included_type && definition.fulltext_enabled() {
                self.index_binary(property, has_mime_type, None, state);
            }
...
impl DocumentMaker<'_> {
    ...
    fn index_binary(
        ...
        property: &PropertyState,
        has_mime_type: bool,
        relative_node_path: Option<&str>,
        state: &mut DocumentState,
    ) {
        let name = relative_node_path.map_or_else(
            || FULLTEXT_FIELD.to_owned(),
            |path| format!("{RELATIVE_NODE_PREFIX}{path}"),
        );
        for value in values_of(property) {
    ...
    fn index_facet(property: &PropertyState, state: &mut DocumentState) {
        let multi_valued = matches!(property.values, PropertyValues::Multiple(_))
            && property.property_type == PropertyType::String;
        ...
        // dimension is recorded whatever its arity.
...
impl DocumentMaker<'_> {
    ...
    fn index_aggregates(
        ...
        let matcher = rule.aggregate.matcher();
        self.walk_aggregate(node, path, rule, &matcher, state)
    }
    ...
    fn walk_aggregate(
        ...
        path: &str,
        rule: &IndexingRule,
        matcher: &crate::index::lucene::documents::aggregate::Matcher<'_>,
        state: &mut DocumentState,
    ) -> IndexResult<()> {
            ...
            match outcome {
                ...
                Match::Aggregate(includes) => {
                    for include in includes {
                        self.aggregate_node(&child, rule, include, state)?;
                    }
                }
            }
            self.walk_aggregate(&child, &child_path, rule, &next, state)?;
        }
        Ok(())
    }

    /// One aggregated node's properties, each a `:fulltext` value — or a
    /// `fullnode:<include path>` one for a `relativeNode` include.
    fn aggregate_node(
        &self,
        node: &NodeState<'_>,
        rule: &IndexingRule,
        ...
        let name = if include.relative_node {
        ...
        for property in node.properties()? {
            ...
            let definition = rule.config_of(&property.name);
