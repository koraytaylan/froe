...
/// serializes dates, floored toward the past.
///
/// The parse itself is [`crate::java::parse_epoch_milliseconds`], which is
/// Jackrabbit's own and is shared with the Lucene document maker. The
/// division is euclidean because `/` truncates toward zero, which would
/// round a pre-epoch instant with a fraction *up* — `None` for anything
/// that does not parse, because a date the store renders strangely must
/// never fail a plan.
