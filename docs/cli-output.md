...
| `index list`, `index definitions` | `inventorying indexes` (nodes — one per definition node read) |
| `index check` | `inventorying indexes` (nodes), then `checking indexes` (nodes — the definitions checked) |
| `index dump` | `dumping index files` (files — every index file streamed out of `:data` and `:suggest-data`) |
| `index import` (plan; `--dry-run` runs the same pass read-only) | no step of its own: the plan reads the input directory's metadata files and the store's definitions, and finishes before a bar would draw |
| `index import` (apply) | `copying index files` (files — every file streamed into `:data`), then `verifying the imported index` (files — every one of them read back out and compared byte for byte before the head moves) |
| `index reindex` (plan; `--dry-run` runs the same pass read-only) | `inventorying indexes` (nodes), then per definition `collecting index entries` (nodes — the counting walk, so the plan can name a number) and `sorting index entries` (entries, only when the walk spilled). A `lucene` definition opens `making index documents` (nodes) for its own counting walk instead |
| `index reindex` (apply) | per definition `collecting index entries` and `sorting index entries` again — the collecting walk this time — then `writing index records` (nodes) and, once over every rebuilt definition, `verifying the rebuilt indexes` (nodes). A confirmed run therefore walks the content twice per definition and no more: once to count, once to collect. The tail adds no walk of its own; it checks the entries it was given against the content they name |
| `index reindex` (apply, a `lucene` definition) | `making index documents` (nodes — one document per included node with a rule), `writing index segments` (index documents — the writer's own merge of what it spilled) and `copying index files` (files — the segment's own file set into `:data`), then the shared `verifying the rebuilt indexes`. The same two-walk rule holds: one counting walk in the plan, one making walk in the apply |
...
| --- | --- | --- | --- |
| Reports never reach standard output. Every command; exercised through `compact --dry-run`, `index reindex --dry-run` and `index import --dry-run`, the three plans an operator confirms a destructive run from, and through `index dump`, whose data an operator pipes. | `reporting_never_reaches_the_standard_output_of_a_compaction_plan`, `reporting_never_reaches_the_standard_output_of_a_reindex_plan`, `reporting_never_reaches_the_standard_output_of_an_import_plan`, `reporting_never_reaches_the_standard_output_of_a_dump` | `Reporter::new` constructs with `std::io::stdout()` instead of `std::io::stderr()`. | The reported and silent standard outputs differ: every progress line is prepended to the plan. Four unrelated export tests also fail, their data streams contaminated. |
| Silence never hides a destructive confirmation. `mutation::confirm`, reached by every mutating command including `index reindex` and `index import`; exercised through an interactive `compact --silent`, through `index reindex` without `--yes`, which plans and cancels naming the flag, and through `index import` without `--yes`, which prints its plan and leaves the store byte-identical. | `silence_never_hides_the_destructive_confirmation_prompt`, `a_scripted_run_without_yes_plans_and_cancels_naming_the_flag`, `an_unconfirmed_import_changes_nothing` | `confirm` writes the prompt through `Reporter::status`, which silence suppresses. | The test times out after 15s waiting for a prompt that never arrives, with the plan printed and the process blocked on an invisible question. |
| The observed twin of an operation returns what the plain spelling returns. `plan_compaction` / `plan_compaction_with_progress`, and the same pairing for `compact`, `backup`, `plan_reindex`, `reindex`, `dump_lucene_indexes`, `plan_lucene_import`, `lucene_import`, `IndexInventory::collect`, and the readers. | `an_observed_plan_equals_an_unobserved_one`, `an_observed_compaction_equals_an_unobserved_one`, `an_observed_backup_equals_an_unobserved_one`, `an_observed_reindex_plan_equals_an_unobserved_one`, `an_observed_reindex_equals_an_unobserved_one`, `an_observed_dump_equals_an_unobserved_one`, `an_observed_import_plan_equals_an_unobserved_one`, `an_observed_import_equals_an_unobserved_one`, `an_observed_inventory_equals_an_unobserved_one` | `plan_compaction` stops delegating to `plan_compaction_with_progress` and appends a warning of its own. | `assertion left == right failed: observation must not change the plan an operator confirms`, with the drifted warning shown in the diff. |
