...
| `index reindex` (plan; `--dry-run` runs the same pass read-only) | `inventorying indexes` (nodes), then per definition `collecting index entries` (nodes — the counting walk, so the plan can name a number) and `sorting index entries` (entries, only when the walk spilled) |
