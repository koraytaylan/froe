...
| Counter index rebuild | `NodeCounterEditor`, `SipHash` | the same entry point; a definition with no `seed` gains one | **Implemented** (beta; a counter on an unresolvable lane is refused by name — froe will not rebuild one and Oak does not rebuild one on a lane whose checkpoint is gone) |
