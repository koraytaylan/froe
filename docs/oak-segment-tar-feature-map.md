...
| `--index-dump` | `LuceneIndexDumper` | `froe index dump` | **Implemented** (**beta** in `v0.12.0`; plan 0008's review is frozen — [its evidence](plans/0008-lucene-index-transport/ARCHITECTURE.md)) |
| `--reindex` | `OutOfBandIndexer`, `IndexUpdate` | `froe index reindex` | **Implemented** for the property family — `property`, `unique`, `reference`, `counter` (**beta** in `v0.12.0`; plan 0007's review is frozen — [its evidence](plans/0007-property-index-reindex/ARCHITECTURE.md)) — and for fulltext-enabled `lucene` definitions (**beta** in `v0.12.0`; plan 0010's review is frozen — [its evidence](plans/0010-lucene-offline-reindex/ARCHITECTURE.md)), which need `--binary-text` because froe extracts no binary text. A froe extension: Oak has no offline reindex that writes the result back, and rebuilds synchronously inside the first commit after startup instead. Refused by name for a Lucene definition: a codec verdict that is not `oakCodec`, a definition-level `valueRegex`, a hybrid or synchronous definition, `similarityTags`, `useInSimilarity`, `dynamicBoost`, `function`, `compatVersion 1`, `maxFieldLength = 0`, two rules typing one `:dv` field differently, an `nt:base` rule with a `nullCheckEnabled` property, a consumer-registered analyzer — and, at document time, a `DATE` value that does not parse. |
| `--index-import` | `IndexImporter`, `IndexDefinitionUpdater` | `froe index import` | **Implemented** (**beta** in `v0.12.0`; plan 0008's review is frozen — [its evidence](plans/0008-lucene-index-transport/ARCHITECTURE.md)). froe replaces oak-run's live bring-up-to-date with a precondition — the directory's checkpoint must resolve to the state the lane resumes from — because an offline tool has no editors to replay commits with. |
...
| `froe digest REPOSITORY [--output FILE] [--baseline FILE] [--exclude-subtree PATH]… [--exclude-property-prefix PREFIX]…` | The repository's content rendered canonically — every node, property, type, arity, value and binary checksum, over the head and every checkpoint — for comparing a store before and after an operation. `--exclude-subtree` omits named content subtrees and stamps the exclusion into the output, so a digest bracketing a confirmed purge excuses the purge and nothing else. `--exclude-property-prefix` omits properties by name at every node and stamps its own header, which is how two indexes built from identical content are compared despite Oak's approximate counters, whose `:count_<uuid>` property names are drawn from a random generator. Oak has no equivalent: `oak-run check` answers whether records parse, not whether the content is the content. |
...
| `froe index dump REPOSITORY --output DIRECTORY [--index PATH]…` | Write Lucene index data to the filesystem in oak-run's layout, read-only against the store. `<output>/index-dumps` is what `froe index import --input` and oak-run's `--index-import-dir` read. `indexer-info.properties` is written only when every dumped definition is on one lane whose checkpoint resolves; otherwise the files are a backup and the reason is named. See [`index.md`](index.md) §6. **Beta** in `v0.12.0`; plan 0008's review is frozen, and [its evidence](plans/0008-lucene-index-transport/ARCHITECTURE.md) is what the label rests on. |
| `froe index import REPOSITORY --input DIRECTORY [--index PATH]… [--dry-run] [--yes]` | Install Lucene index data built out of band back into a stopped store: the mapped index directories, the bookkeeping Oak's own importer leaves, one head move under the lock. In place of oak-run's live bring-up-to-date it requires a precondition — the directory's checkpoint must resolve to the state the definition's lane resumes from — so a directory built at any other state is refused naming both. `index-definitions.json` must describe the definitions as the store holds them. No checkpoint is released and `:suggest-data` is never imported. `--dry-run` previews it read-only without the lock. See [`index.md`](index.md) §7. **Beta** in `v0.12.0`; plan 0008's review is frozen, and [its evidence](plans/0008-lucene-index-transport/ARCHITECTURE.md) is what the label rests on. |
| `froe index reindex REPOSITORY [--index PATH]… [--dry-run] [--yes] [--work-directory DIRECTORY] [--from-head] [--sort-budget-mebibytes N] [--binary-text marker\|skip] [--pre-extracted-text-directory DIRECTORY]` | Rebuild flagged property, unique, reference, counter and fulltext-enabled `lucene` indexes offline, from the state Oak's own editors would index — one head move under the lock, one journal line, the content tree untouched. `--dry-run` previews it read-only without the lock. `--from-head` is consulted only for a definition whose lane cannot be resolved; a counter and a Lucene definition are then **reset** — the hidden children removed and `reindex` raised — so Oak's own next cycle rebuilds from scratch rather than doubling what a froe rebuild would have left. `--binary-text` is required for every Lucene definition, binaries or not, because froe extracts none; `--pre-extracted-text-directory` reads Oak's own store first. See [`index.md`](index.md) §5. **Beta** in `v0.12.0`; plans [0007](plans/0007-property-index-reindex/ARCHITECTURE.md)'s and [0010](plans/0010-lucene-offline-reindex/ARCHITECTURE.md)'s reviews are frozen, and their evidence is what the label rests on. |
...
| Feature | Java | froe | Status |
| --- | --- | --- | --- |
| Property index rebuild (`ContentMirrorStoreStrategy`) | `PropertyIndexEditor`, `ContentMirrorStoreStrategy` | `writer::index::reindex`, `plan_reindex`, `PreparedReindex` | **Implemented** (**beta** in `v0.12.0`; plan 0007's review is frozen — [its evidence](plans/0007-property-index-reindex/ARCHITECTURE.md)) |
...
| Counter index rebuild | `NodeCounterEditor`, `SipHash` | the same entry point; a definition with no `seed` gains one | **Implemented** (beta; a counter on an unresolvable lane is **reset** rather than rebuilt — Oak's own replay would double a rebuilt one — and the interop suite's `property_reindex` scenario ends in Oak's own from-scratch rebuild) |
| Lucene index rebuild | `LuceneIndexEditor`, `FulltextDocumentMaker`, `OakAnalyzer` | the same entry point, with `ReindexOptions::with_binary_text_policy`; `index::lucene::analysis`, `index::lucene::documents` | **Implemented** as a library capability (**beta** in `v0.12.0`; plan 0010's review is frozen — [its evidence](plans/0010-lucene-offline-reindex/ARCHITECTURE.md)). Documents are made from the definition's own rules and analyzed with Oak's own chain, a segment is written with plan 0009's writer and copied into `:data`. Refused by name: a codec verdict that is not `oakCodec`, a definition-level `valueRegex`, a hybrid or synchronous definition, a definition parked on the `async-reindex` lane, `function`, `dynamicBoost`, `useInSimilarity`, `similarityTags`, `compatVersion 1`, a `maxFieldLength` of zero, an `nt:base` rule with a `nullCheckEnabled` property, two rules typing one `:dv` field differently, a `unique` or `sync` property definition, a consumer-registered analyzer — and, at document time, a `DATE` value that does not parse. **froe extracts no binary text**: the policy supplies Oak's own pre-extracted text or the extraction-error marker. |
| Lucene **segment writer** as a library capability | `IndexWriter` over the `oakCodec` composition — `Lucene41` postings, `Lucene45` doc values, `Lucene46` field infos and segment info, `Lucene42` norms, uncompressed `Lucene40` stored fields | `index::lucene::writer::LuceneIndexWriter`, `Document`, `Field` | **Implemented** (**beta** in `v0.12.0`; plan 0009's review is frozen — [its evidence](plans/0009-lucene-segment-writer/ARCHITECTURE.md)). Writes one compound segment and the commit over it, for **fulltext-enabled** definitions, which is the scope `oakCodec` covers; analysis is the caller's, since Oak's analyzer chain is Oak's. |
| Lucene index **transport** — out to the filesystem and back into a stopped store | `LuceneIndexDumper`, `IndexImporter`, `IndexDefinitionUpdater` | `index::lucene::dump_lucene_indexes`, `writer::lucene_import`, `writer::plan_lucene_import`, `writer::PreparedLuceneImport`; `froe index dump`, `froe index import` | **Implemented** (**beta** in `v0.12.0`; plan 0008's review is frozen — [its evidence](plans/0008-lucene-index-transport/ARCHITECTURE.md)). The import replaces oak-run's live bring-up-to-date with a precondition: the directory's checkpoint root must equal the definition's lane checkpoint root by identity, so no catch-up is owed. Recorded departures: no checkpoint is released, `:suggest-data` is never imported, and `refresh` is not copied. |
