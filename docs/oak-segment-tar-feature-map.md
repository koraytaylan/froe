...
| `--reindex` | `OutOfBandIndexer`, `IndexUpdate` | `froe index reindex` | **Implemented** for the property family — `property`, `unique`, `reference`, `counter` (beta until plan 0007's review freezes) — and for fulltext-enabled `lucene` definitions (beta until plan 0010's review freezes), which need `--binary-text` because froe extracts no binary text. A froe extension: Oak has no offline reindex that writes the result back, and rebuilds synchronously inside the first commit after startup instead. Refused by name for a Lucene definition: a codec verdict that is not `oakCodec`, a definition-level `valueRegex`, a hybrid or synchronous definition, `similarityTags`, `useInSimilarity`, `dynamicBoost`, `function`, `compatVersion 1`, `maxFieldLength = 0`, two rules typing one `:dv` field differently, an `nt:base` rule with a `nullCheckEnabled` property, a consumer-registered analyzer — and, at document time, a `DATE` value that does not parse. |
| `--index-import` | `IndexImporter`, `IndexDefinitionUpdater` | `froe index import` | **Implemented** (beta until plan 0008's review freezes). froe replaces oak-run's live bring-up-to-date with a precondition — the directory's checkpoint must resolve to the state the lane resumes from — because an offline tool has no editors to replay commits with. |
