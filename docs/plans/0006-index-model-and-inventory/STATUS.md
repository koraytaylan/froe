...
- **Exceptions:** task 0612's chain-to-completion-sentinel clause is unverified. The chain ran as far as `cleanup` — ten phases, with `read`, `judge_smoke`, `index_inventory` and `commit` passing in order — and was then interrupted three times by the host's memory watchdog at three different phases, after which another workload on the shared host bound port 8080. Nothing failed; the runs were stopped. Run `scripts/interop-fixture.sh` to its sentinel on an unshared host before relying on the chain as evidence.
